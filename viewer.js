...
const uniforms={uPointSize:{value:.005},uBrightness:{value:1},uGlowStrength:{value:.35},uGlowRadius:{value:.45},uTime:{value:0},uMotion:{value:1},uScale:{value:1},uRadius:{value:1},uBass:{value:0},uMid:{value:0},uHigh:{value:0},uEnvelope:{value:0},uGlobalWave:{value:1.8},uLocalWave:{value:2.2},uMotionSpeed:{value:.55},uAudioInfluence:{value:1.35},uFireflyAmount:{value:.45},uFireflyRange:{value:1.2},uFireflySpeed:{value:1},uFireflyOrbit:{value:1},uFireflyNoise:{value:1}};
let motionTarget=1;

const material=new THREE.ShaderMaterial({uniforms,transparent:true,depthWrite:false,vertexColors:true,vertexShader:`
uniform float uPointSize,uGlowRadius,uTime,uMotion,uScale,uRadius,uBass,uMid,uHigh,uEnvelope,uGlobalWave,uLocalWave,uMotionSpeed,uAudioInfluence,uFireflyAmount,uFireflyRange,uFireflySpeed,uFireflyOrbit,uFireflyNoise;
...
function saveCurrentView(){
  ...
  updateFocusMarker(homeTarget);flyOriginCamera.copy(camera.position);flyOriginTarget.copy(controls.target);flyProgress=0;
}
function ensureQuickSaveViewButton(){
  if(document.querySelector('#quickSaveViewBtn'))return;
  const btn=document.createElement('button');
  btn.id='quickSaveViewBtn';
  btn.type='button';
  btn.title='將目前視角設為旋轉中心';
  btn.setAttribute('aria-label','儲存目前視角與旋轉中心');
  btn.innerHTML='<span aria-hidden="true"></span>';
  btn.style.cssText='position:relative;display:grid;place-items:center;width:38px;height:38px;padding:0;border:1px solid rgba(255,255,255,.18);border-radius:999px;background:rgba(5,7,7,.58);color:rgba(255,255,255,.55);cursor:pointer;backdrop-filter:blur(10px);transition:color .18s ease,border-color .18s ease,background .18s ease,transform .18s ease';
  const icon=btn.firstElementChild;
  icon.style.cssText='position:relative;display:block;width:17px;height:17px;border:1.5px solid currentColor;border-radius:50%';
  const dot=document.createElement('i');
  dot.style.cssText='position:absolute;left:50%;top:50%;width:5px;height:5px;border-radius:50%;background:currentColor;transform:translate(-50%,-50%)';
  icon.appendChild(dot);
  btn.onmouseenter=()=>{btn.style.color='#fff';btn.style.borderColor='rgba(255,255,255,.42)'};
  btn.onmouseleave=()=>{if(!btn.classList.contains('is-saved')){btn.style.color='rgba(255,255,255,.55)';btn.style.borderColor='rgba(255,255,255,.18)'}};
  btn.onclick=()=>{
    saveCurrentView();
    btn.classList.add('is-saved');
    btn.style.color='#fff';
    btn.style.borderColor='rgba(255,255,255,.58)';
    ...
    setTimeout(()=>{
      ...
      btn.style.background='rgba(5,7,7,.58)';
    },900);
  };
...
function makeCompactRange(label,min,max,step,value,onInput,digits=2){
  const row=document.createElement('label');row.style.cssText='display:grid;grid-template-columns:92px 1fr 42px;align-items:center;gap:7px;margin-top:8px';
  const name=document.createElement('span');name.textContent=label;
  const input=document.createElement('input');input.type='range';input.min=min;input.max=max;input.step=step;input.value=value;
  const out=document.createElement('output');out.textContent=Number(value).toFixed(digits);out.style.textAlign='right';
  input.oninput=()=>{const v=Number(input.value);out.textContent=v.toFixed(digits);onInput(v)};
  row.append(name,input,out);return {row,input,out};
}

function ensureQuickFlyButton(){
  if(document.querySelector('#quickFlyBtn'))return;
...
function ensureV12Controls(){
  if(!settings||document.querySelector('#v12Controls'))return;
  const box=document.createElement('section');box.id='v12Controls';box.style.cssText='margin-top:12px;padding-top:10px;border-top:1px solid rgba(255,255,255,.12);font:10px/1.45 ui-monospace,SFMono-Regular,Menlo,monospace;color:rgba(255,255,255,.72)';
  const title=document.createElement('div');title.textContent='FIREFLY / FLY THROUGH';title.style.cssText='color:#fff;letter-spacing:.08em;margin-bottom:4px';
  const speed=makeCompactRange('FIREFLY SPEED',.1,6,.05,uniforms.uFireflySpeed.value,v=>uniforms.uFireflySpeed.value=v,2);
  const orbit=makeCompactRange('FIREFLY ORBIT',.1,12,.1,uniforms.uFireflyOrbit.value,v=>uniforms.uFireflyOrbit.value=v,2);
  const noise=makeCompactRange('FIREFLY NOISE',0,8,.05,uniforms.uFireflyNoise.value,v=>uniforms.uFireflyNoise.value=v,2);
